    ...
    'Files' => [
        'csv_fetch' => [
            ...
            'vars' => [
                ...
                ['name' => 'offset', 'label' => 'Start from row (0 = first data row)', 'default' => '0', 'type' => 'number'],
                ['name' => 'limit', 'label' => 'Max rows (0 = all)', 'default' => '0', 'type' => 'number']
            ]
        ]
    ]
];
...
$batch_process_templates = [
    ...
    'Files' => [
        'csv_row' => [
            ...
            'code' => "<?php\n// \$item comes from \"CSV: fetch rows (batch)\".\n// It should already be a prepared row (array/object) that you can display or transform.\n\$row = \$item;\nif (!is_array(\$row) && !is_object(\$row)) {\n    echo Ajax_Snippets_Table::render(['error' => 'Row is not an array/object', 'row' => \$row], 'CSV Row');\n    return;\n}\n\n// Customize output as needed.\n/*\nif (!empty(\$row['status']) && \$row['status'] === 'active') {\n    // do something\n}\n*/\n\necho Ajax_Snippets_Table::render(\$row, 'CSV Row');",
            'vars' => []
        ]
    ]
];
